...
test("submits text and renders provider output as text",async()=>{
  const fetchMock=vi.spyOn(globalThis,"fetch").mockResolvedValueOnce(new Response(JSON.stringify({status:"ok",session:{id:"s1",kind:"advisor",title:"Tư vấn tài chính",status:"active",created_at:"2026-09-12T00:00:00Z",updated_at:"2026-09-12T00:00:00Z"},message:{id:"m1",session_id:"s1",run_id:"r1",role:"user",text:"Phân tích",created_at:"2026-09-12T00:00:00Z"},run:{id:"r1",session_id:"s1",kind:"advisor",status:"completed",request_text:"Phân tích",response_text:"<b>An toàn</b>",attempts:1,draft_ids:[]}}),{status:202,headers:{"Content-Type":"application/json"}}));
  render(<AgentScreen online onDraftReady={()=>{}}/>);fireEvent.click(screen.getByRole("button",{name:"Hỏi tài chính"}));fireEvent.change(screen.getByLabelText("Yêu cầu cho trợ lý"),{target:{value:"Phân tích"}});fireEvent.click(screen.getByRole("button",{name:/Gửi yêu cầu/}));
