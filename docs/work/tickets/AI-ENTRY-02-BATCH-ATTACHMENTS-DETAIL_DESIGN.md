...
- Live synthetic image test passes private S3 PUT/readback (1,027,882 bytes; 62 ms) and OCR (3.815 s; mock total recognized). In the captured five-case model rerun, all calls failed strict draft-schema validation: 0/15 fields, LLM p50 15.089 s/p95 21.7 s, transfer-safety gate failed. The OCR + receipt extraction pipeline measured 25.516 s. S3 is verified; AI extraction compatibility is tracked separately under AI-ENTRY-01. See [BUG-001](../bugs/BUG-001-s3-presigned-get-signature.md), [ADR-007](../../decisions/ADR-007-aws-s3-presigning.md) and [validation evidence](../VALIDATION_MATRIX.md#ai-entry-02-live-provider-evaluation-2026-09-22).
