...
- Cold-start guard (2026-09-23): oMLX model loading/prefill can exceed 30 seconds even with bounded output. `modelTimeout` is now 90 seconds and the outer extraction budget is 180 seconds; a regression test requires the outer budget to exceed the model budget. The backend was restarted from `.env.local` and a post-restart text smoke returned HTTP 200 with one review proposal.
