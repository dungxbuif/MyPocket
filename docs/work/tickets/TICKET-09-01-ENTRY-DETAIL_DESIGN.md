...
- Frontend tests/design/build pass; detailed commands and browser fixture evidence in [page spec](../../design/pages/assistant/README.md). Live Chrome through FE origin verified normal click/manual form and AI chat configuration/empty state, without writing owner transactions.
...
- Runtime diagnostic (2026-09-23): the configured oMLX Qwen endpoint exposed a 1,000,000-token default when MyPocket omitted `max_tokens`; an image request then remained in generation until the 30-second model timeout and was logged as `stage=model`, `code=model_request`, `provider_status=0`. The extraction request now sends `max_tokens=2048`, `temperature=0`, and `chat_template_kwargs.enable_thinking=false`. Direct strict-schema smoke with a 5.4k-token prompt completed in about 3 seconds; no provider fallback or automatic retry was added.
- Cold-start guard (2026-09-23): model loading/prefill for multi-image batches can exceed 90 seconds even with bounded output. After a real 503 at ~91 seconds with health still 200, `modelTimeout` is now 180 seconds and the outer extraction/usecase budget is 210 seconds; a regression test requires a multi-image-safe model budget and the outer budget to exceed it. The backend was restarted from `.env.local` and health returned HTTP 200. A fresh owner submission is required because the failed request remains an idempotent terminal result.
