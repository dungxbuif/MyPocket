...
Latest owner decision supersedes the picker design below: use BaseSelect while creating, immutable type after save. Canonical [wallet page specification](../../design/pages/wallets/README.md) now covers list/create/edit, base mapping, type-specific behavior and official Money Lover sources. Runtime still has prior picker; select replacement and visual UAT remain pending.
...
Source: [Add Wallet](../../design/references/add-wallet/README.md), [wallet selector](../../design/references/wallet-selector/README.md). The second image selects existing wallets, not wallet types; type selection adapts its list-row composition to the three supported types. Existing-wallet scope selection is not silently implemented as part of this change.
