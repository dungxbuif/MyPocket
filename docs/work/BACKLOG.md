...
2026-09-24 wallet/ledger UI follow-up: savings detail has an existing goal/progress spec; Travel Mode is still an event-linkage draft and not a new wallet type. [UI-LEDGER-WEEK](tickets/UI-LEDGER-WEEK-DETAIL_DESIGN.md) is in_review after owner correction: basic uses the same Week/Custom summary/list UI as `Tổng cộng`, filtered by wallet; goal shows complete savings history, and provisional credit remains read-only. Basic selection and week navigation were verified on the local Chrome page; goal selection/mobile visual UAT remain open. Credit has business rules, but its detailed statement/payment UI/API contract remains a separate design gap; do not mark credit complete. No deployment has been performed for this slice.

2026-09-23: owner requires AI entry to suggest reviewable drafts, auto-merge clear repeated screenshot rows and prefill inferred/default wallets rather than refusing the batch. [Review-first correction](bugs/AI-ENTRY-REVIEW-FIRST-DETAIL_DESIGN.md): in_review, regression_verified locally. Proposal count is no longer capped in parser/usecase/repository; only provider response/transport envelopes remain. Nine stored OCR attachments yield 30 drafts; held-out overlap/partial-row and 31-draft tests pass. Go tests/race/build pass and local API restarted. Next: owner upload/review UAT with a new request; do not claim all source rows reconciled or production deployed.
2026-09-23: diagnostics/usage follow-up adds privacy-safe AI stage logs, optional internal model-usage persistence (`AI_STORE_USAGE`, enabled by default), explicit `user_instruction` filtering guidance, and a user-isolation audit. Streaming is deliberately deferred until an async/polling contract is separately designed; current JSON extraction remains bounded at 180s model/210s overall.
...
| active | TICKET-02-02 | product | high-risk | Implement atomic internal transfer between wallets | Owner request | in_review | [Design](tickets/TICKET-02-02-TRANSFER-DETAIL_DESIGN.md), [Transaction page](../design/pages/transactions/README.md) | Data model, Public contract/API, Authorization, Existing behavior | Owner visual UAT; follow-up pair edit/delete/idempotency | Migration 000016, endpoint, handler/PostgreSQL tests, integrated roundtrip and real Chromium Quick Add E2E pass; owner sign-off remains. |
