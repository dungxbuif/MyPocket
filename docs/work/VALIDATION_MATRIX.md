...
| REQ-F-004 | PHASE-003 | TICKET-008..010 | Offline mirror, outbox, sync, tombstones, and conflict review | yes | yes | yes | yes | yes | yes | in_progress | TICKET-008 frontend proof covers IndexedDB mirror/outbox, legacy migration/quarantine, durable sequence, offline cached hydration, optimistic transaction queueing, wallet/category queue primitives, degraded read-only UI, and PWA shell offline reload in [PHASE-003 offline sync verification](test-verification/PHASE-003-offline-sync.md); backend sync, conflict inbox, replay E2E, and UAT remain pending |
