...
Latest 2026-09-11: full backend race suite/vet, 161 frontend tests, typecheck/production build and **87/87 browser tests pass**. [Mobile layout proof](phases/MOBILE-OVERLAY-DETAIL_DESIGN.md) resolves chart-induced viewport overflow; controlled 0/100 long-name wallets and seven large-value report labels pass on all three profiles. Only the controlled layout spec blocks service workers for route interception; real PWA/offline suites retain them. Physical-device/provider and production verification remain open.
