...
| Reports/QuickAdd | QuickAddSheet, AssistantComposer, AssistantResultCard, base controls/cards/text | QuickAdd and one-shot AI review are mounted/API-backed. AI review keeps the bulk “Lưu tất cả” action in the form-sheet header so it remains available while proposal rows scroll. Broader reports/charts and financial Q&A remain pending; monthly detail is the current live numeric report. Transfer creation is opened from the transactions three-dot menu. |

## Feedback bubble

`FeedbackFloatingBubble` is mounted by `MobileAppShell`, so it follows the authenticated app shell rather than a single route. Its trigger uses the shared surface `IconButton` treatment (not a one-off brand variant) while remaining a floating circular affordance above navigation. Opening the bubble starts a best-effort `html2canvas` capture of the `mypocket-app-root` ref. Elements marked `data-feedback-overlay="true"` are excluded, and the user may remove the preview before submitting. The form uses the shared base modal, inputs, select, textarea, button, icon button, and status components; the optional PNG is sent through the feedback multipart API.
