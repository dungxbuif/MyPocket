...
- Reserve an OCR adapter boundary, but do not implement a concrete OCR provider until the owner supplies the repository skill and provider contract.
...
- Verify receipt capture, durable offline bytes, upload/download, retry, authorization, quota behavior, and physical Safari/PWA behavior.
...
- Publish Docusaurus endpoint contracts, curl examples, lifecycle guides, sync semantics, error handling, and a repository-versioned agent skill.
...
- Keep OCR as an interface plus disabled capability state until the owner-provided skill defines implementation details.
...
- PostgreSQL is authoritative. Redis, S3-compatible storage, OpenAI-compatible providers, push, pricing, and future OCR are adapters with explicit failure behavior.

New AI code belongs behind a small provider interface and an application service that reads authorized user context and emits typed drafts/analysis. It must not embed provider calls in HTTP handlers or mutate finance repositories directly.
...
5. OCR is explicitly disabled behind its adapter pending the supplied skill; bank and voice remain out of scope.
...
This specification supersedes earlier milestone boundaries that deferred AI, export, account lifecycle, and production operations. It explicitly removes bank ingestion from the active scope and leaves OCR implementation pending the owner-provided skill.
