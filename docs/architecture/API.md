...
Same request ID/payload does not repeat extraction. Different payload with same ID, stale edits and conflicting terminal decisions return 409; invalid input 400, unavailable/unconfigured provider 503, wrong owner 404, missing auth 401. There is currently no per-user AI/OCR usage cap; provider-cost controls are a future provider-selection concern. Expired processing lease shows interruption; no automatic resubmission. Environment capability booleans indicate configuration, not successful provider authentication or quality.
