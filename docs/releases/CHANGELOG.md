...
- Fixed receipt upload validation to use the detected file signature instead of trusting an empty or incorrect browser MIME header; valid PNG/JPEG/PDF uploads now reach OCR while spoofed content remains rejected. Added a shared floating feedback bubble that captures the current mounted app view (excluding its overlay) and submits an optional private screenshot through the feedback API. Transfer creation is now exposed from the transactions three-dot menu. A live receipt image now completes OCR and Qwen extraction in local staging; blank/unsupported content remains a safe OCR rejection.
- Fixed the live OCR source contract: image submissions now use validated Base64 and PDF submissions use OCR Platform's own private presign flow, instead of sending MyPocket S3 signed URLs that OCR Platform returns as 404. Originals remain private in MyPocket storage and the LLM still receives OCR text only.
- Integrated OCR bounded wait mode: `POST /v1/documents` now sends `mode: "wait"` and `waitTimeoutMs: 15000`, consumes terminal `200` results inline, and falls back to the existing document poll loop for `202` receipts. The MyPocket adapter tests cover inline completion, legacy unknown-field negotiation, and PDF presign submission. Live OpenAPI and an authenticated Base64 receipt now confirm the provider accepts the new contract.
- Moved AI Entry’s bulk `Lưu tất cả` action into the sticky bottom-sheet header so it remains available while proposal rows scroll. Standardized the floating feedback trigger on the shared surface `IconButton` token and added browser/base-contract regressions.
- Fixed recurring Qwen/oMLX AI-entry 503s caused by the provider's 1,000,000-token default when `max_tokens` was omitted. Extraction now caps output at 2,048 tokens, disables thinking, and uses deterministic temperature zero; the failure is covered by provider contract tests and a live text/receipt smoke after restart.
- Increased the AI-entry model timeout to 90 seconds and total extraction budget to 180 seconds so the first request after a local oMLX restart can finish model loading/prefill instead of failing at the old 30-second boundary. Added a regression guard for the timeout relationship; no retry or ledger auto-write was introduced.
