...
paths:
  ...
  /api/v1/transactions/{id}:
    patch:
      summary: Sửa giao dịch với optimistic concurrency
      security:
        - cookieAuth: []
        - csrf: []
        - bearerAuth: []
...
`base_version` phải lấy từ `version` gần nhất đã đọc. Server khóa bản ghi và từ chối version cũ trước khi đảo/apply lại delta kế toán. Client API key cũng gửi field này như browser; bearer request không cần CSRF.

Lưu trữ dùng `POST /api/v1/transactions/{id}/archive` với JSON `{ "base_version": <version> }`; stale version trả `409 VERSION_CONFLICT` và không đảo số dư.
