...
`plaintext` chỉ trả về 1 lần duy nhất. Không ghi key vào log hay source code.
...
OAuth và health endpoint không cần bearer. API key hợp lệ có thể quản lý key cùng chủ sở hữu; thao tác xóa/reset tài khoản vẫn bắt buộc recent browser cookie authentication.

Audit API vẫn kiểm tra email đúng `AUDIT_VIEWER_EMAIL` sau xác thực, dù principal đến từ cookie hay API key.

Bearer API bị giới hạn mặc định 120 request/phút theo user/key. `429` có `Retry-After`; Redis không sẵn sàng làm Bearer request fail-closed bằng `503`.
