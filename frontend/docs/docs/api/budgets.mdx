...
```
spent_vnd = SUM(transactions.amount_vnd) WHERE transactions.budget_id = budget.id
...
`budget_categories` vẫn là metadata/hint cho UI, nhưng không tự động kéo mọi giao dịch cùng category vào tiến độ. Người dùng hoặc tích hợp API phải gắn `budget_id` trực tiếp vào giao dịch expense, draft expense hoặc recurring expense. Phép tính phần trăm dùng số nguyên lớn nội bộ và bão hòa ở `MaxInt64`, không được wrap khi dữ liệu cực trị. Update/archive bắt buộc gửi `base_version` lấy từ `budget.version`; version cũ trả `409 VERSION_CONFLICT` và không ghi đè dữ liệu mới hơn.
