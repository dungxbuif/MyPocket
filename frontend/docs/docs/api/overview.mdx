---
title: Tổng quan API
---

# API Overview

> Audit 2026-09-10: tài liệu đang được đối chiếu với router và kiểu dữ liệu. Export/AI/OCR/account reset-delete chưa có implementation. Confirm/reject recurring draft đã có API và kiểm tra version/idempotency. Không coi các YAML minh họa rời rạc hoặc diagram là OpenAPI máy đọc đã được kiểm định; kiểm tra trạng thái từng nhóm API. Có bearer auth không đồng nghĩa mọi chức năng Money Lover đã được expose.

**Base URL:** `/api/v1`

**Content-Type:** `application/json`

## Authentication methods

| Method | Header/Cookie | Áp dụng cho |
|--------|-------------|-------------|
| **Cookie** | `mypocket_auth` (HttpOnly, Secure, SameSite=Lax) | Browser (UI và automation) |
| **API Key** | `Authorization: Bearer mpk_...` | Third-party, server-to-server |

Mutations bằng cookie yêu cầu thêm `X-CSRF-Token` header. Với Bearer API key, không dùng CSRF.

### Quy tắc xác thực theo route

- Tất cả endpoint nghiệp vụ user-owned (`wallets`, `categories`, `transactions`, `planning`, `reports`, `assets`, `files`, `sync`, `notifications`, `budgets`) chấp nhận cả cookie và Bearer API key.
- Không chấp nhận bearer cho: `GET /api/v1/auth/*`, `POST /api/v1/auth/logout`, `GET /api/v1/health/*`, docs pages, và route quản lý API key.

### Quy tắc phân quyền

- `user_id` luôn lấy từ context auth (cookie hoặc API key), không được đọc từ payload.
- Mọi truy vấn/mutation user-owned phải lọc theo `user_id` hiện tại.
- Truy cập tài nguyên không thuộc scope của caller trả `403 FORBIDDEN`.
- Truy cập thiếu auth hợp lệ trả `401 AUTH_REQUIRED`.

## Response envelope

```json
{
  "status": "ok",
  "correlation_id": "req_abc123",
  "wallets": [ ... ]
}
```

Lỗi:
```json
{
  "status": "error",
  "error": { "code": "VALIDATION_FAILED", "message": "Description" },
  "correlation_id": "req_abc123"
}
```

## Common errors

| Error | HTTP | Ý nghĩa |
|-------|------|---------|
| `AUTH_REQUIRED` | 401 | Chưa login |
| `FORBIDDEN` | 403 | Không có quyền |
| `VALIDATION_FAILED` | 400 | Input không hợp lệ |
| `NOT_FOUND` | 404 | Object không tồn tại |
| `VERSION_CONFLICT` | 409 | base_version cũ |
| `IDEMPOTENT_REPLAY` | 200 | Mutation đã được xử lý |
| `ASSET_OVERSELL` | 400 | Bán vượt quá số lượng |
| `UPLOAD_REJECTED` | 400 | File không hợp lệ |
| `INTERNAL_RETRYABLE` | 503 | Lỗi tạm thời, retry |
| `INTERNAL_FAILURE` | 500 | Lỗi không xác định |

## Middleware stack

```mermaid
graph LR
    Req[Request] --> CORS[CORS]
    CORS --> Corr[Correlation ID]
    Corr --> Auth[Auth Context<br/>Cookie/Bearer/CSRF]
    Auth --> Obs[Observability<br/>Audit + Log]
    Obs --> Route[Route Handler]
```

## Sequence diagram — Create transaction

```mermaid
sequenceDiagram
    participant Client
    participant API
    participant DB
    participant S3

    Client->>API: POST /transactions<br/>Idempotency-Key: uuid<br/>{type, amount, wallet, category}
    API->>DB: Check idempotency key
    alt Already exists
        DB-->>API: Return cached response
        API-->>Client: 200 IDEMPOTENT_REPLAY
    else New mutation
        API->>DB: INSERT transaction
        API->>DB: UPDATE wallet balance
        API->>DB: INSERT sync_change
        API->>DB: INSERT audit_event
        DB-->>API: Success
        API-->>Client: 201 Created
end
```

## Redis cache và Audit

Redis là lớp cache/hint cho xác thực:

- Lưu hint API key hash → `user_id`; PostgreSQL vẫn xác nhận key active trên từng Bearer request
- Lookup token/cookie đã ký
- TTL ngắn; miss/lỗi/stale Redis không thể thay thế PostgreSQL (nguồn dữ liệu chính)

Audit event **không** truy xuất từ Redis như nguồn chính. Các event thay đổi trạng thái/sự kiện bảo mật được ghi vào PostgreSQL append-only.

`/api/v1/audit/access` và `/api/v1/audit/events` là route đọc-only riêng biệt. Chỉ tài khoản có email trùng chính xác `AUDIT_VIEWER_EMAIL` mới được truy cập sau xác thực thành công.

## Sequence diagram — Offline sync flow

```mermaid
sequenceDiagram
    participant PWA
    participant IndexedDB
    participant API
    participant PG

    Note over PWA,IndexedDB: User creates transaction offline
    PWA->>IndexedDB: save to outbox
    PWA->>IndexedDB: save to mirror
    
    Note over PWA,PG: User comes online
    PWA->>API: POST /sync/mutations<br/>[{mutation_id, entity, payload}]
    API->>PG: Check idempotency
    API->>PG: Apply accounting
    alt Version conflict
        API-->>PWA: conflict + server payload
        PWA->>IndexedDB: save conflict
        PWA-->>User: Show conflict resolution UI
    else Success
        API-->>PWA: applied
        PWA->>IndexedDB: update mirror
    end
    
    PWA->>API: GET /sync/changes?after=CURSOR
    API-->>PWA: {changes, next_cursor}
    PWA->>IndexedDB: apply changes to mirror
```
