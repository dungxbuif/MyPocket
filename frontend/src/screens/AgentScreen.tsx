...
import { effectiveAgentStatus, loadAgentRun, submitAgentMessage, type AgentRunView } from "../app/agent";
...
export function AgentScreen({ online, onDraftReady }: { online: boolean; onDraftReady: () => void }) {
  ...
  React.useEffect(()=>{
    ...
    let cancelled=false;
    const timer=window.setInterval(()=>{void loadAgentRun(run.id).then(next=>{if(cancelled)return;setRun(next);}).catch(error=>{if(!cancelled)setFailure(operationFailure(error,"Chưa cập nhật được tiến độ. Hãy thử lại."));});},1500);
    return()=>{cancelled=true;window.clearInterval(timer)};
  },[run?.id,run?.status,onDraftReady]);
  ...
  async function submit(){
    ...
    try{const next=await submitAgentMessage({kind,message:message.trim(),image:image??undefined});setRun(next);setMessage("");setImage(null)}catch(error){setFailure(operationFailure(error,"Chưa gửi được yêu cầu. Nội dung vẫn được giữ để thử lại."))}finally{setBusy(false)}
  }
  ...
  return <section className="content-stack agent-screen" aria-label="Trợ lý tài chính">
    ...
    {run?<div className="agent-thread"><AgentMessage role="user" text={run.request_text}/>{status?<AgentRunStatus status={status}/>:null}{run.response_text?<AgentMessage role="agent" text={run.response_text}/>:null}{run.draft_ids.length>0?<Card><p>Đã tạo bản nháp để bạn kiểm tra, sửa, xác nhận hoặc từ chối.</p><Button variant="outline" onClick={onDraftReady}>Mở bản nháp</Button></Card>:null}{run.error_code?<p className="offline-warning" role="alert">Yêu cầu không hoàn tất. Mã: {run.error_code}</p>:null}</div>:null}
  </section>;
}
