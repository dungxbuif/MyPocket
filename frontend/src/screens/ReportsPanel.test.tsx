...
describe("ReportsPanel", () => {
  ...
  it("renders six server-derived periods for comparison", async () => {
    vi.stubGlobal("fetch", vi.fn(async () => ok()));
    render(<ReportsPanel {...props} />);
    filters("comparison");
    await userEvent.click(screen.getByRole("button", { name: "Xem báo cáo" }));
    const history = await screen.findByRole("table", { name: "So sánh 6 kỳ" });
    expect(within(history).getAllByRole("row")).toHaveLength(7);
    expect(within(history).getByText("400.000 đ")).toBeVisible();
    expect(within(history).getByText("900.000 đ")).toBeVisible();
  });
