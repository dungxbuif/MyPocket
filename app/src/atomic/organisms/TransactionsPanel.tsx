...
export function TransactionsPanel({ refreshKey = 0, onChanged }: { refreshKey?: number; onChanged: () => void }) {
  ...
  return (
    <section className="space-y-3">
      ...
      {creating ? <QuickAddSheet onClose={() => setCreating(false)} onSaved={() => { setCreating(false); setLocalRefresh((value) => value + 1); onChanged(); }} /> : null}
    </section>
  );
}
