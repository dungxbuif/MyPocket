...
export function BaseBottomSheet({ title, closeLabel, onClose, children }: { title: string; closeLabel: string; onClose: () => void; children: ReactNode }) {
  return <div className="fixed inset-0 z-40 flex items-end justify-center bg-slate-950/30 px-0" role="presentation" onMouseDown={onClose}>
    <section role="dialog" aria-modal="true" aria-label={title} className="max-h-[88vh] w-full max-w-[430px] overflow-y-auto rounded-t-3xl bg-white px-4 pb-[calc(env(safe-area-inset-bottom)+20px)] pt-3 shadow-[0_-10px_30px_rgb(15_23_42/0.12)]" onMouseDown={(event) => event.stopPropagation()}>
