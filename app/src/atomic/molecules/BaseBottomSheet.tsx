...
export function BaseBottomSheet({ title, closeLabel, onClose, children }: { title: string; closeLabel: string; onClose: () => void; children: ReactNode }) {
  ...
  return <div className="fixed inset-0 z-40 flex items-end justify-center bg-heading/30 px-0" role="presentation" onMouseDown={onClose}>
    <section ref={panel} tabIndex={-1} role="dialog" aria-modal="true" aria-label={title} className="max-h-[88vh] w-full max-w-[430px] overflow-y-auto rounded-t-3xl bg-card px-4 pb-[calc(env(safe-area-inset-bottom)+20px)] pt-3 shadow-sheet" onMouseDown={(event) => event.stopPropagation()}>
